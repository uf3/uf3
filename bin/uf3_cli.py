...
def parse_data_files(settings, data_files):
    ...
    data_coordinator = io.DataCoordinator(atoms_key=settings.get('data', {}).get('keys', {}).get('atoms_key', default_settings['data']['keys']['atoms_key']),
                energy_key=settings.get('data', {}).get('keys', {}).get('energy_key', default_settings['data']['keys']['energy_key']),
                force_key=settings.get('data', {}).get('keys', {}).get('force_key', default_settings['data']['keys']['force_key']),
    ...
    if 'data' in settings and 'sources' in settings['data'] and 'path' in settings['data']['sources']:
        ...
        if isinstance(path, str):
            if os.path.isfile(path) and os.path.splitext(path)[-1] in accepted_file_formats:
                ...
                logging.error("Specified data file does not exist or is not a supported format. Exiting.")
                sys.exit(1)
        ...
        # Check if list of files
            ...
            for file in path:
                if os.path.isfile(file) and os.path.splitext(file)[-1] in accepted_file_formats:
                    ...
                    logging.warning(f"Specified data file {file} does not exist or is not a supported format. Skipping.")

        # Throw error if not a string or list
        else:
            logging.error("Specified data files is not a string or list. Exiting.")
            sys.exit(1)

    # Fallback to finding data files in the current directory
    else:
        logging.warning("No data file specified in settings. Searching for data files in current directory.")
...
def create_bspline_config(settings, degree, chemical_system):
    ...
    r_min = settings.get('basis', {}).get('r_min', default_settings['basis']['r_min'])
    ...
    if isinstance(r_min, float):
        ...
        r_min_map = {i: r_min if len(i) == 2 else [r_min, r_min, r_min * 2] for i in interactions}
    elif isinstance(r_min, dict):
        for i in interactions:
            if not i in r_min:
                logging.warning(f"No minimum cutoff specified for interaction {i}. Defaulting to {default_settings['basis']['r_min']}.")
                r_min[i] = default_settings['basis']['r_min']
        r_min_map = r_min
    else:
        logging.error("Minimum cutoff must be a dict, string, integer, or float. Exiting.")
        sys.exit(1)
    ...
    r_max = settings.get('basis', {}).get('r_max', default_settings['basis']['r_max'])
    ...
    if isinstance(r_max, float):
        ...
        r_max_map = {i: r_max if len(i) == 2 else [r_max, r_max, r_max * 2] for i in interactions}
    elif isinstance(r_max, dict):
        for i in interactions:
            if not i in r_max:
                logging.warning(f"No maximum cutoff specified for interaction {i}. Defaulting to {default_settings['basis']['r_max']}.")
                r_max[i] = default_settings['basis']['r_max']
        r_max_map = r_max
    else:
        logging.error("Maximum cutoff must be a dict, string, integer, or float. Exiting.")
        sys.exit(1)
    ...
    resolution = settings.get('basis', {}).get('resolution', default_settings['basis']['resolution'])
    if isinstance(resolution, str):
        resolution = int(resolution)
    if isinstance(resolution, int):
        logging.info(f"Using resolution of {resolution} for all interactions.")
        resolution_map = {i: resolution if len(i) == 2 else [resolution, resolution, resolution * 2] for i in interactions}
    elif isinstance(resolution, dict):
        for i in interactions:
            if not i in resolution:
                logging.warning(f"No resolution specified for interaction {i}. Defaulting to {default_settings['basis']['resolution']}.")
                resolution[i] = default_settings['basis']['resolution']
        resolution_map = resolution
    else:
        logging.error("Resolution must be a dict, string, or integer. Exiting.")
        sys.exit(1)
    ...
    # Basis
    leading_trim = settings.get('basis', {}).get('leading_trim', default_settings['basis']['leading_trim'])
    ...
    trailing_trim = settings.get('basis', {}).get('trailing_trim', default_settings['basis']['trailing_trim'])
...
def get_degree(settings):
    if not 'degree' in settings:
        logging.warning(f"No degree specified in settings. Defaulting to {default_settings['degree']}.")
...
def load_data(settings):
    ...
    import pandas as pd
    pickle_path = settings.get('data', default_settings['data']).get('pickle_path')
...
def collect(args):
    ...
    if args['settings']:
        ...
        # Set verbosity
        logging.getLogger().setLevel(settings.get('verbose', default_settings['verbose']))
        ...
        # Parse data files
        df_data = parse_data_files(settings, data_files)
        df_data.to_pickle(settings.get('data', default_settings['data']).get('pickle_path'))
    ...
    # If neither have been specified, scan the current directory for data files
        ...
        logging.info("No data file specified. Searching for data files in current directory.")
...
def featurize(args):
    ...
    if settings.get('features', {}).get('parallel', default_settings['features']['parallel']) == 'python':
        ...
        logging.error("Only python multiprocessing is supported at this time. Exiting.")
        sys.exit(1)

    filename = get_features_filename(settings)

    batch_size = settings.get('features', {}).get('batch_size', default_settings['features']['batch_size'])
    ...
    table_template = settings.get('features', {}).get('table_template', default_settings['features']['table_template'])
    ...
    progress = settings.get('data', {}).get('progress', default_settings['data']['progress'])
...
def get_features_filename(settings):
    filename = settings.get('features', {}).get('filename', default_settings['features']['features_path'])
...
def fit(args):
    ...
    ridge_1b = float(settings.get('learning', {}).get('regularizer', {}).get('ridge_1b', default_settings['learning']['regularizer']['ridge_1b']))
    curvature_1b = float(settings.get('learning', {}).get('regularizer', {}).get('curvature_1b', default_settings['learning']['regularizer']['curvature_1b']))
    ridge_2b = float(settings.get('learning', {}).get('regularizer', {}).get('ridge_2b', default_settings['learning']['regularizer']['ridge_2b']))
    curvature_2b = float(settings.get('learning', {}).get('regularizer', {}).get('curvature_2b', default_settings['learning']['regularizer']['curvature_2b']))
    ridge_3b = float(settings.get('learning', {}).get('regularizer', {}).get('ridge_3b', default_settings['learning']['regularizer']['ridge_3b']))
    curvature_3b = float(settings.get('learning', {}).get('regularizer', {}).get('curvature_3b', default_settings['learning']['regularizer']['curvature_3b']))
    weight = float(settings.get('learning', {}).get('weight', default_settings['learning']['weight']))
    ...
    filename = get_features_filename(settings)
    energy_key = settings.get('data', {}).get('keys', {}).get('energy_key', default_settings['data']['keys']['energy_key'])
    progress = progress = settings.get('data', {}).get('progress', default_settings['data']['progress'])
    batch_size = settings.get('learning', {}).get('batch_size', default_settings['learning']['batch_size'])
    if isinstance(batch_size, str):
        try:
            batch_size = int(batch_size)
        except:
    ...
    regularizer = bspline_config.get_regularization_matrix(ridge_1b=ridge_1b, curvature_1b=curvature_1b, ridge_2b=ridge_2b, curvature_2b=curvature_2b, ridge_3b=ridge_3b, curvature_3b=curvature_3b)
    ...
    model_path = settings.get('learning', {}).get('model_path', default_settings['learning']['model_path'])
...
def main(args):
    ...
    parser_collect = subparsers.add_parser('collect', help='collect data and export to pickled file')
    parser_collect.set_defaults(func=collect)
    group = parser_collect.add_mutually_exclusive_group()
    group.add_argument('settings', type=str, help='settings .yaml file', nargs='?', default=False)
    group.add_argument('-f', '--file', type=str, help='single data file to pickle to \'data.pkl\'', default=False)

    parser_featurize = subparsers.add_parser('featurize', help='compute feature vectors')
    parser_featurize.set_defaults(func=featurize)
    parser_featurize.add_argument('settings', type=str, help='settings .yaml file', default=False)

    parser_fit = subparsers.add_parser('fit', help='fit model to data')
    parser_fit.set_defaults(func=fit)
    parser_fit.add_argument('settings', type=str, help='settings .yaml file', default=False)
