...
def get_curvature_penalty_matrix(n_features, magnitude=1, ridge=0):
    """
    Generates additive regularization matrix for linear regression
        using curvature penalty and, optionally,  L2 (ridge) penalty.
    ...
    Args:
        ...
        magnitude (float): curvature regularization strength (multiplicative).
    ...
    """
    reg_diag = np.eye(n_features) * 2 * magnitude
    reg_minus = np.eye(n_features, k=-1) * -magnitude
    reg_plus = np.eye(n_features, k=1) * -magnitude
...
def get_penalty_matrix_2D(L, M, ridge=0):
    """
    Generates additive regularization matrix for linear regression
        using curvature penalty and, optionally,  L2 (ridge) penalty.
...
def get_penalty_matrix_3D(L, M, N, ridge=0):
    """
    Generates additive regularization matrix for linear regression
        using curvature penalty and, optionally,  L2 (ridge) penalty.
        Curvature penalty here applies to coefficients that are spatially
